###################################################
# Partly copy from Bosca Ceoil Blue               #
...
func _init() -> void:
    if _instance:
        Logger.error("PopupManager: Only one instance of PopupManager is allowed.")
...
func _draw_catcher() -> void:
    _click_catcher.draw_rect(Rect2(Vector2.ZERO, _click_catcher.size), Color("21212180"))
...
# grow_direction 是处理最小尺寸大于当前尺寸时的行为的。
...
func create_positioned_popup(popup: PopupControl, position: Vector2, direction: Direction, blocking: bool) -> void:
    if has_popup(popup):
        Logger.error("PopupManager: Popup %s is already shown." % [popup])
        return
    if popup.get_parent():
        Logger.error("PopupManager: Popup %s must be unparented before it can be shown." % [popup])
        return
...
func translate_anchored_popup(popup: PopupControl, anchor_position: Vector2, direction: Direction) -> void:
    if not has_popup(popup):
        Logger.error("PopupManager: Popup %s is not shown." % [popup])
        return
