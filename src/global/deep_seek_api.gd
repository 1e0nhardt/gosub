...
func json_to_clips(json_path: String) -> bool:
    ...
    # -ml 1
    if SettingHelper.get_setting_value("/transcribe/whisper.cpp/smart_split"):
    ...
    if SettingHelper.get_setting_value("/transcribe/whisper.cpp/smart_split"):
        for new_clip in clips:
            if new_clip.second_text.length() > SettingHelper.get_setting_value("/transcribe/whisper.cpp/smart_punctuation_threshold"):
    ...
    #     if clips[ind].second_text.length() > SettingHelper.get_setting_value("/transcribe/whisper.cpp/sentence_min_words"):
    #         if ind < clips.size() - 1 and clips[ind + 1].start - clips[ind].end > SettingHelper.get_setting_value("/transcribe/whisper.cpp/sentence_max_gap_time") * 1000:
    ...
    deepseek_chat_translate.set_system_prompt.call_deferred(SettingHelper.get_setting_value("/llm/common/prompt/translate"))
    EventBus.ai_translate_progress_updated.emit.call_deferred(0)
    var source_contents := ""

    var clips_one_chat: int = SettingHelper.get_setting_value("/llm/common/clips_per_chat")
